...
impl StatsTracker {
    ...
    fn ingest_batch(&self, batch: &DataBatch) -> BatchIngestReport {
        ...
        self.duplicate_txs
            .fetch_add(duplicate_tx, Ordering::Relaxed);
    ...
    fn snapshot(&self) -> StatsSnapshot {
        StatsSnapshot {
            ...
            duplicate_txs: self.duplicate_txs.load(Ordering::Relaxed),
...
fn log_stats(previous: &StatsSnapshot, current: &StatsSnapshot, interval: Duration) {
    ...
    let topics_summary = if topic_delta.is_empty() {
        "n/a".to_string()
    } else {
        topic_delta
            .into_iter()
